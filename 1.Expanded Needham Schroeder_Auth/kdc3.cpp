...
int main(int argc, char *argv[])    {
       ...
       if(newfd < 0) cout<<"\nError accepting connection..";
		...
		DES_set_key((C_Block *)keystrb3, &ksb3);	
	 	...
	 	numbytes=recv(newfd,recbuf,SOCBUFSIZE,0);				//receives from Alice N1,Alice wants Bob, msg_from_Bob
	    recbuf[numbytes+1]='\0';
	    cout<<"\n3.Received from Alice:"<<recbuf;
	    
		char de[]="/////";
		char *c=tokenizer(recbuf,de); string str1_n1((char *)c);	    //N1
		c=tokenizer(NULL,de); string str2((char *)c);			//Alice wants Bob
    	c=tokenizer(NULL,de); string str3((char *)c);			//msg_from_Bob	    
	    
	    
	    memcpy(ivecb, (C_Block *)ivecstrb, sizeof(ivecb));
	    memset(in, 0, sizeof(out));
	    memset(out, 0, sizeof(back));
	    memcpy(out,str3.c_str(),BUFSIZE);							//msg_from_Bob
		len = strlen((char *)out)+1;
		DES_ede3_cbc_encrypt(out, back, len, &ksb1, &ksb2, &ksb3, &ivecb, DES_DECRYPT);  //decrypt from Alice msg_from_bob - Kb(Nb) 
		cout<<"\nDecrypted Kb(Nb):"<<back;
	    string msg_from_bob((char *)back);					   // store Nb
	  //  cout<<"\nDecrypted Text Kb(Nb) after memcopy into char[]:"<<msg_from_bob<<"\n";
	    recbuf[0]='\0';
	    memset(out, 0, sizeof(out));
	    memset(back, 0, sizeof(back));	 
		
        //Now KDC sends Alice ticket to bob, Ka(Kab)
		string kab1_str=keystrab1; string kab2_str=keystrab2; string kab3_str=keystrab3;	//shared keys (3 keys) to Alice
        ...
        memcpy(ivecb, (C_Block *)ivecstrb, sizeof(ivecb));	   		
	    memcpy(in, msg_from_bob.c_str(), BUFSIZE);												//Nb decrypted above
	    len = strlen((char *)in)+1;
	    DES_ede3_cbc_encrypt(in, out, len, &ksb1, &ksb2, &ksb3, &ivecb, DES_ENCRYPT);    //encrypt Nb using Kb
	    memcpy(iveca, (C_Block *)ivecstra, sizeof(iveca));
	    //cout<<"\nKb(Nb):"<<out;
	    //cout<<"\nits len:"<<strlen((char*)out);
	    len = strlen((char *)out)+1;   
	    DES_ede3_cbc_encrypt(out, back, len, &ksa1, &ksa2, &ksa3, &iveca, DES_ENCRYPT);  //encrypt Kb(Nb) using Ka
	    string ka_kb_nb_str((char *)back);	
	    cout<<"\nKa(Kb(Nb)):"<<ka_kb_nb_str;
        //cout<<"\nits len:"<<ka_kb_nb_str.length();
											//send Alice Ka(Kb(NB))
	    memset(in, 0, sizeof(in));
	    memset(out, 0, sizeof(out));
	    memset(back, 0, sizeof(back));
	    ...
	    string ka_kb_alice_str((char *)back);											//ka_kb_kab3_str
		cout<<"\nka_kb_alice_str:"<<ka_kb_alice_str;  					
	    //cout<<"\ninlen:"<<strlen((char *)in);
	    //cout<<"\noutlen:"<<strlen((char *)out);        
		//cout<<"\nlen:"<<ka_kb_alice_str.length();	    
	    
    
string kdc_reply=ka_kab1_str+"/////"+ka_kab2_str+"/////"+ka_kab3_str+"/////"+ka_kb_kab1_str+"/////"+ka_kb_kab2_str+"/////"+ka_kb_kab3_str+"/////"+ka_kb_nb_str+"/////"+ka_n1_str+"/////"+ka_bob_str+"/////"+ka_kb_alice_str;
	    cout<<"\n4.KDC to Alice:"<<kdc_reply;

...
//parse and decrypt message from KDC
void decrypt_kdc_msg(char s2[]){
	   ...
	   //cout<<"\noutlen:"<<strlen((char *)out);	
		...
		cout<<"\nDecrypted Ka(Kb(Alice)):"<<kb_alice_g;
	    memset(out, 0, sizeof(out));
	    memset(back, 0, sizeof(back));
...
void newsoc(int port_num2, int mypipe){
		...
		string alice_to_kdc=s1+"/////"+s2;		           //N1,Alice wants Bob
		cout<<"\n1.Alice to KDC:"<<alice_to_kdc;
...
int main(int argc, char *argv[])    {
       ...
       newfd=accept(sockfd,(struct sockaddr *)&thieraddr,&sin_size);	//newfd to communicated with Bob
       ...
       //open another connection to KDC
        ...
        decrypt_kdc_msg(rec);
   	    if(alice_n1check_g==alice_n1_g) cout <<"\nN1 equals. KDC Verified";		//Alice verifies KDC
	   ...
	   // 64 bits of random nonce
		...
		memset(out, 0, sizeof(out));
//		memcpy(ivecab, (C_Block *)ivecstrab, sizeof(ivecab));
		string ticket_kab_n2=kb_kab1_g+"/////"+kb_kab2_g+"/////"+kb_kab3_g+"/////"+kab_n2+"/////"+kb_alice_g;			//ticket_kab_n2
		cout<<"\n3. Alice sends Kab(ticket,N2) to Bob:"<<ticket_kab_n2;
		sendbuf[0]='\0';
...
//		kab_n2.clear();
		...
		cout<<"\n5.Encrypted Kab(N3-1):"<<kab_n3_mi;
		cout<<"\nlen:"<<kab_n3_mi.length();
		memset(in, 0, sizeof(in));
		memset(out, 0, sizeof(out));
